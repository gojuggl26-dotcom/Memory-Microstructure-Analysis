...
板に置かれた指値注文の当初数量(`orig_sz`)は L2 の `lifecycle` 表にある。

> **2026-08-28 訂正**: ここには元々「lifecycle は DEEP_ARCHIVE にあり復元しないと
> 読めない」と書いてあったが、それは誤りだった。8 銘柄ぶんの L2 が手元にあり
> (`E:/hlpipe/l2/` と `C:/Users/ii562/hl-l4-pipeline/data/l2_v99/`)、
> `build_order_age.py` はそれを直接読んでいる。**この節が下で採っている代理変数は、
> 当時この表が使えないと思い込んでいたための迂回であって、最善手ではない。**
> 指値注文のサイズを見るなら lifecycle の `orig_sz` を直接使うべきである。

以下は当時の記述。手元にある約定記録から作れる
