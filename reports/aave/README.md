...
| レポート | 内容 |
|---|---|
| 本ページ | 金利・利用率・弾性の構造と、利用率が 2 つあること |

清算ヒートマップは未着手。借り手の列挙(`Borrow` イベント全史から 85,707 アドレス、
103 コール・222 秒)と Multicall3 の一括読み(600 件/コール・1.63 秒)までは
実測で確認済み。Morpho と違い 1 人が複数の担保と債務を持つので、価格の族ごとに
倍率 m を置いて `HF(m) = 1` を解く必要がある(eMode の扱いも要る)。
