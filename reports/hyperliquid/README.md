...
| 銘柄ごとの分析を読みたい | [`xyz:MU`](MU/README.md) / [`xyz:DRAM`](DRAM/README.md) / [`xyz:INTC`](INTC/README.md) / [`xyz:AMD`](AMD/README.md) / [`xyz:KIOXIA`](KIOXIA/README.md) / [`xyz:SKHX`](SKHX/README.md) / [`xyz:SMSN`](SMSN/README.md) / [`xyz:SNDK`](SNDK/README.md) |
| 銘柄を横並びで比べたい | [7 銘柄の横並び](cross_coin_report.md) |
| メイカーとして出す価値があるか | [7 銘柄のメイカー検証](mm_all_report.md) |
| [1 分足の平均回帰(20 分 SMA 基準)](mrev_report.md) | 1 分足の mid が 20 分 SMA へどれだけ戻るかを、基準価格 4 通り(microprice / EWMA / SMA20 / モデル fair)× 自己相関 × AR(1) の半減期 × 分散比で測ったもの。**同じ欠測・ボラ・ティック幅のランダムウォークを帰無対照に置く**のが要点で、φ=0.92・半減期 8 分はほぼ全部が移動平均の副産物。実際に平均回帰しているのは `xyz:SKHX` と `xyz:SMSN` の 2 銘柄だけで、**それも費用には届かない**。 |
| 価格は平均回帰するか | [1 分足の平均回帰(20 分 SMA 基準)](mrev_report.md) |
| 相場つき(ボラ × 方向)で切ると何か出るか | [1 秒足の 6 レジーム判定](regime_report.md) |
| 特徴量を全部まとめて見たい | [7 銘柄 × 84 本の十分位分析](decile_all_report.md)(2 銘柄 × 229 本の版は [こちら](decile_report.md)) |
...
分析済みの銘柄は `xyz:MU`(主対象)・`xyz:DRAM`・対照の `xyz:INTC` / `xyz:AMD`、
および MU の分析一式を適用した `xyz:KIOXIA` / `xyz:SKHX` / `xyz:SMSN` / `xyz:SNDK` です。
主な結果を抜き出します。括弧内はその根拠となるレポートです。

- **板の姿は銘柄で 20 倍違うのに、特徴量の「効き方」は 7 銘柄でほぼ同一。**
  主要 15 特徴量 × 1 秒先の前向き順位相関を銘柄ごとのベクトルにすると、
  21 ペア全ての相関が **+0.95〜+1.00**。δ = microprice − mid の逆張りは
  7 銘柄すべてで負(−0.16〜−0.28)、mid 建てでは 7 銘柄すべてで正に反転する。
  **MU で見つけた構造は銘柄の個性ではなく、この取引所の板の性質**である。
  → [7 銘柄の横並び](cross_coin_report.md)

- **予測力の山のホライズンは、流動性が薄いほど遅くへずれる。**
  約定が速い MU・SNDK・INTC は 500ms、中間の AMD・SKHX・SMSN は 1 秒、
  ほぼ約定の無い KIOXIA は 10 秒(最大 \|r\| = 0.359、ただし幅 67 ティックの
  板の「更新の遅さの写像」であって取れる予測力ではない)。
  → [7 銘柄の横並び](cross_coin_report.md)

- **`xyz:KIOXIA` の板は 2026-06-25 まで存在しない。** fills は 5 月からあるが、
  bbo は 99 ファイル中 52 が空。分析は 47 日で行い、板は幅 67 ティック・
  最良 0.19 枚の実質未稼働。→ [`xyz:KIOXIA` 分析索引](KIOXIA/README.md)
...
- **★ 候補 2 の「見送り」判定は誤りだった。落ちていたのは約定条件ではなく
  信号の時間解像度である。**板を 5 秒格子から **100 ms 格子**で作り直すと、
  発注時点での信号の古さが **2.49 秒 → 0.050 秒**になる(実測した半減期は
  約 250 ms なので、前者は強度の 3% しか残っていなかった)。作り直すと
  **段階 1(全候補)から勾配が立ち(Q5−Q1 +0.040 → +0.609)、約定条件を
  通っても往復損益まで残る(+0.212 → +1.833)**。往復損益との相関は
  +0.0045 → **+0.1078** と 24 倍になった。門に足したときの日次総額は
  **+40.63 ± 10.59(t=+3.83)、39 日中 33 日で勝つ**。同じ変数を 2.49 秒
  古くした版は **−28.37(有意に害)**であり、これがそのまま最良の
  帰無対照になっている。先読み検査(1 セル寝かせ)でも 85% 残る。
  **説明変数の時間解像度が仮説の検定可能性そのものを決めていた。**
  ただし五分位は全部負のままで単独戦略ではなく、遅延 65 ms では水準が
  ゼロと区別できない(差は +40.2、t=+3.95 で保つ)。
  → [信号が古かった](MU/mu_cand2_fresh_report.md)

- **条件つきの出し直しも効かなかった。価値は「板に居ること」ではなく
  「最良に居ること」である。**気配が動くたびに無条件で並び直す従来の規則に対し、
  「維持 / 取消して再発注」を条件で選ぶ規則を 13 通り試したところ、
  **12 通り中 10 通りで従来が Bonferroni 水準で有意に優った**(残り 2 通りは
  差が検出できない)。機構がはっきりしている — **注文を置きっぱなしにすると
  約定率は 5.10% から 8.01% へ上がるのに損益は下がる**。増える約定は
  「市場が自分の値段まで掃いてきた」約定、つまり逆選択そのものである。
  順番の価値は**現在の最良における順番**の価値であって、古い値段における
  順番の価値ではない。
  → [条件つきの出し直しは効かない](MU/mu_requote_report.md)
...
- **清算エンジンは約 3 秒の刻みで動き、自分で自分を呼ぶ。** `xyz:MU` の 99 日で
  実際に強制決済された建玉 **14,889 件 \$57.00M** を全件取り出すと、清算の **80.4%** は
  別の清算と同じミリ秒に起き、ラウンドどうしの間隔は**最小 2.558 秒**で 3 秒・6 秒・9 秒に
  峰が立つ。次の刻みにも続く確率は **28.9%** で、同じ 1 時間の中で置き直した帰無 **2.25%** の
  **13 倍**。**2 ラウンド以上続いた連鎖 504 本は 100% が同じ向き**だった。
  ★**清算された枚数の 35.8% しか建玉(OI)は減らない** — 残りは相手方が新規に建てるので
  移るだけである。→ [実際に清算された建玉のヒートマップ](MU/mu_liquidation_report.md)

- **24 時間動く perp は、原資産の寄り付きに対して「速い追随者」であって「発見者」ではない。**
  `xyz:MU` の 13:29:59 の値段は MU の寄り値を誤差 中央 **11.6bp** で当てる(夜間の値動きは
  中央 389.5bp)。しかし**同時刻のプレマーケットは 5.2bp で 2.2 倍正確**で、
  プレマーケットを統制すると perp の増分 $`R^2`$ は **+0.0002**(p=0.094)しか残らない。
  ★**寄り付きの瞬間(13:30:00 UTC)に perp が跳ねる** — 直後 1 秒で 13.8bp、
  非立会日の同時刻の **271 倍**。寄り値との差の **70% を 1 分で埋めに行く**(t=6.2)。
  唯一 perp が勝つのは**現物が世界のどこでも動かない 00〜08 UTC の 8 時間**
  (1 日の分散の 24.7% を作る)だけで、08:00 のプレマーケット開始までに追い越される。
  → [24 時間動く perp は米国市場の寄り付きを先に知っているか](MU/mu_price_discovery_report.md)

- **その夜の値段を作っているのは、株価指数先物とアジアのメモリ現物である。**
  現物が世界のどこでも動かない 00:00〜08:00 UTC の値動きは、**指数先物だけで
  $`R^2`$ = 0.773**、アジアのメモリ現物(サムスン・SK ハイニックス・キオクシア・
  東エレ・TSMC)を足して **0.850**(Δ +0.078、p=0.004)。アジアの中で個別に
  t > 2 なのは **SK ハイニックスだけ**。★1 日 48 個の 30 分刻みすべてで測ると、
  **1 秒の跳ねは米国の寄りが 1 位**(13.79bp、背景の最大 2.82bp の 4.9 倍)で
  アジアの寄りは 18 位。ただし「直後 5 分 ÷ 直前 5 分」では**東京・ソウルの寄りが
  1 位**(3.79)— 夜の価格発見は一撃ではなく、数時間かけて連続的に起きる。
  → [米国が閉まっている夜、値段を動かしているのはどこか](MU/mu_night_discovery_report.md)

- **ボリンジャーバンドの三本線で、板の流れは確かに変わる。ただし閾値にはならない。**
  BB(20, 2)を 1 分足に載せると、OFI は帯の位置に沿って単調に −1.26 → +1.28 動き、
  \|OFI\| は両端で中間線の **2.2 倍**になります。大半は「直前に動いたから」という
  同語反復ですが、**同じ 1 分リターンの層で帯の内側の足を引いた超過 OFI は、
  到達の 10 分前から +0.12 → +0.43 と積み上がる**ので、帯に固有の成分は残ります。
  ★**減衰は明確** — 到達した分の OFI 1.14 が次の 1 分で 0.12 まで落ち、
  OFI 自身の持続(ラグ 1 自己相関 +0.181)だけなら 0.21 のはずで、**ふだんの 2 倍前後**。
  一方で**反転は無く**(到達後の超過 OFI は ±0.05)、**突破時の OFI の大きさで
  「その突破が続くか」を当てることもできません**(AUC 0.496〜0.513、区間が 0.5 を跨ぐ)。
  → [ボリンジャーバンドの三本線と板の特徴量](MU/mu_bollinger_report.md)

- **特徴量を全部・全ホライズンで並べても、費用線を越えるものは 1 つも無い。**
  `xyz:MU` と `xyz:INTC` の特徴量 229 本を十分位に切り、100ms〜60 秒の 9 ホライズン ×
  mid/micro(1 銘柄あたり最大 4,122 通り)と突き合わせました。Bonferroni を通るのは
  **27.9% / 35.9%**(プラセボは **0.0% / 0.1%**)と豊富にありますが、
  **その最大の \|D10 − D1\| は 2.785bp / 1.630bp** で、往復費用 **2.83bp** に届きません。
  2.83bp を越えるセルは 36 / 180 本ありますが、**そのどれも有意ではありません**
  (\|t\| の中央値 1.63 / 1.03)。効く時間帯は **300ms〜5 秒の台地**で、10 秒以降は落ちます。
  ★**どのホライズンでも mid 建てのほうが micro 建てより通過本数が多く**、
  代表例 `delta1_bp` は mid で正・micro で負(10 秒で +0.575 対 −2.672bp)。
  2 銘柄で有意になったセルの符号は 10 秒・60 秒で **100% 一致**します。
  → [特徴量 229 本 × 9 ホライズンの十分位分析](decile_report.md)

- **所有 7 銘柄すべてに広げても、費用線を越えるものは 1 つも無い。**
  最良気配と約定だけで作れる 84 本を 7 銘柄 × 9 ホライズン × mid/micro
  (8,712 通り)に当てました。Bonferroni を通るのは **40.0〜61.9%**、
  プラセボは **7 銘柄すべて 0.0%(0 本)**。効くのは **OFI** で、銘柄をまたいで
  再現した mid 建て 344 通りのうち **122 通りが OFI ファミリー**、上位 12 位は全部 OFI です。
  ★**効くホライズンは板の厚さで変わる** — 中央スプレッド 1.1bp の `xyz:MU` は
  **100ms で既に 70%** が有意なのに対し、13.9bp の `xyz:KIOXIA` は 100ms で 24% しかなく
  **30〜60 秒でようやく 61%** に達します。
  ★**費用は銘柄ごとに違う**(往復 2.66bp〜15.49bp)ので単一の 2.83bp を当てるのは誤りで、
  判定も \|D10 − D1\| ではなく **片側 max(\|D1\|,\|D10\|)** で行うべきです。
  そうすると **4,530 本の有意なセルのうち費用を越えるものはゼロ**、いちばん惜しい
  `xyz:SKHX` の `ofi_ewma → mid_60s` でも **費用の 45%** どまりでした。
  ★micro 建ての巨大な効果(`xyz:KIOXIA` で −17.5bp)は **microprice の定義から出る
  機械的な平均回帰**で、執行できません。
  → [7 銘柄 × 84 本の十分位分析](decile_all_report.md)

- **メイカーとして出す価値のある銘柄は、7 銘柄のどれにも無い。**
  `xyz:MU` で組んだ在庫つきメイカー(両側に指値・\|q\|≤1 で FIFO 相殺・BBO 追随)を
  設定ごと 7 銘柄へ当てました。★**半スプレッドは約定するまでに消える** —
  出したときの半スプレッド ×2 の **21〜113%** が、約定するまでの mid の動きで失われ、
  `xyz:MU`(113%)と `xyz:AMD`(102%)は**約定した瞬間にもう負け**ています。
  残りは保有中の drift(−1.4〜−6.8bp)が食う。手数料は 0.176bp で誤差。
  ★黒字なのは **板が薄く半スプレッドが 16.2bp ある `xyz:KIOXIA` だけ**(+1.976bp/組)で、
  能力の差ではなく板の差です。★入口の門を往復損益で学習すると全銘柄で改善しますが、
  **評価期間で 0 を超えるのは `xyz:KIOXIA` だけ**。しかも
  **同じ本数を無作為に通す門でも +1.84 → +2.84bp 上がる**ので、
  門の価値は「門あり − 門なし」ではなく「門あり − 無作為の門」で測るべきです。
  ★**最後は数量で消えます** — `xyz:KIOXIA` の BBO には中央 **\$66** しか並んでおらず、
  \$100 のクオートで +1.98 → −1.13bp へ反転。最良の組み合わせでも**年 \$2.2 万**で、
  **t=2.36 / 符号検定 p=0.36 と有意ではなく**、設備費は引いていません。
  → [7 銘柄のメイカー検証](mm_all_report.md)

- **1 分足の mid はほぼランダムウォーク。「半減期 8 分」は移動平均の副産物。**
  20 分 SMA からの乖離 \|D_t = mid − SMA20\| を AR(1) に当てると
  φ=0.90〜0.92・半減期 6.6〜8.6 分・R²=0.81〜0.86 と、いかにも強い平均回帰に見えます。
  ★しかし**同じ欠測・同じボラ・同じティック幅のランダムウォークが
  φ=0.920〜0.923・半減期 8.3〜8.7 分を出す**ので、これは移動平均残差の定義から来る値です。
  対照との差が有意なのは `xyz:SKHX`(z=−4.09)と `xyz:SMSN`(z=−3.95)の 2 銘柄だけ。
  ★**分散比なら見えます** — `xyz:SKHX` は VR(20)=0.852(対照 0.980、z=−5.2)、
  `xyz:SMSN` は 0.856(同 0.983、z=−3.5)。反転は 1 つのラグに集中せず
  **19 ラグに薄く広がる**ので、ρ(1) を見ても分かりません。
  ★**約定値で測ってはいけません** — bid–ask bounce だけで ρ(1) が
  `xyz:KIOXIA` −0.098 まで負に出ます(mid だと逆に +0.029)。
  ★**平均回帰は米国立会時間のほうが強く**(8 銘柄中 7 銘柄)、
  ★**費用を引くと残りません**(1sd の乖離で取れる片道の粗利 0.56〜2.88bp 対
  往復費用 2.65〜15.51bp)。
  → [1 分足の平均回帰(20 分 SMA 基準)](mrev_report.md)

- **レジームは実在する状態で、方向も当てる。それでも費用を引くと 88 通り全部が負。**
  1 秒足の mid を **ボラ 2 × 方向 3 = 6 レジーム**(ボラは直近 30 分の 55/70 分位で
  ヒステリシス、方向は効率比 ER と符号つき標準化 DS の AND)に分け、11 銘柄へ当てました。
  ★**雑音の呼び分けではありません** — 分布はそのままで**時間構造だけ壊した
  ブートストラップ対照**と比べると、R1 の平均継続は 1.4〜2.6 倍長く、自己遷移確率は
  11 銘柄すべてで対照を上回ります。★**方向レジームは当たります** — 198 通りのうち
  Bonferroni を通る **87 通りの全部が R2〜R5**(方向つき)で、方向のない R1・R6 は
  **0/33・0/33**。プラセボ(対照ラベルで切る)は中央 0.014bp。
  ★**ただし気配が疎な銘柄ではボラ判定が壊れます** — `xyz:GOLD` は気配が動く秒が
  10.3% しかなく、R6(高ボラ/両建て)が 61.3%(対照 19.1%)を占め、
  将来ボラの高/低比は **0.73 と逆転**します。
  ★**費用を引くと 88 通り全部が負** — 判定の 1 秒後にテイカーで建てて 60/300 秒持つと、
  最良でも `xyz:GOLD` の **−1.35bp/取引**。総額は **−279 万 USD** で、
  対照の「何もしない」(0 USD)に負けます。ただし「無作為に同数建てる」対照
  (−325 万 USD)には **+46 万 USD 勝つ**ので、**シグナル自体は実在します**。
  粗利/費用の比は 0.10〜0.34(中央 0.28)で、**費用の安い銘柄へ逃げても比は改善しません**。
  → [1 秒足の 6 レジーム判定](regime_report.md)
...
**清算は薄く広くではなく、点で起きる。**
6 時間 × \$10 の格子 28,908 セルのうち埋まっているのは 0.86% だけで、
ロングの清算は価格の谷に、ショートの清算は山に貼りつきます。
...
→ [実際に清算された建玉のヒートマップ](MU/mu_liquidation_report.md)
...
| 銘柄コード | 原資産 | 分析索引 | 備考 |
|---|---|---|---|
| `xyz:MU` | マイクロン・テクノロジー | **[開く](MU/README.md)** | 分析済み(このリポジトリの主対象。レポート 56 本) |
...
| `xyz:KIOXIA` | キオクシア | **[開く](KIOXIA/README.md)** | 分析済み(MU の分析一式を適用)。**板の稼働開始が 2026-06-25 で 47 日しかない**。幅 67 ティックの実質未稼働の板 |
| `xyz:SKHX` | SK ハイニックス | **[開く](SKHX/README.md)** | 分析済み(同上)。取引件数と建玉は 7 銘柄で最大 |
| `xyz:SMSN` | サムスン電子 | **[開く](SMSN/README.md)** | 分析済み(同上) |
| `xyz:SNDK` | サンディスク | **[開く](SNDK/README.md)** | 分析済み(同上)。回転 3.2 回/日・清算 49 件/日の投機の板 |

これに加えて、**メモリではない銘柄を対照として**入れています。
...
| `xyz:AMD` | AMD(ロジック、ファブレス) | **[開く](AMD/README.md)** | 対照 2(MU の分析一式を適用)。約定が疎な板 |

7 銘柄を同じ物差しで並べた比較は
**[7 銘柄の横並び](cross_coin_report.md)** にあります。
...
- [`xyz:DRAM` 分析索引](DRAM/README.md)
- [`xyz:INTC` 分析索引](INTC/README.md)
- [`xyz:AMD` 分析索引](AMD/README.md)
- [`xyz:KIOXIA` 分析索引](KIOXIA/README.md)
- [`xyz:SKHX` 分析索引](SKHX/README.md)
- [`xyz:SMSN` 分析索引](SMSN/README.md)
- [`xyz:SNDK` 分析索引](SNDK/README.md)
...
| [特徴量 229 本 × 9 ホライズンの十分位分析](decile_report.md) | `xyz:MU` と `xyz:INTC` の特徴量ライブラリ全 229 本を十分位に切り、100ms〜60 秒の 9 ホライズン × mid/micro の将来リターンと突き合わせたもの。十分位の境目は前日の分布、標準誤差は日でクラスタ、帰無対照は 1 営業日ずらし。**有意なセルは 27.9% / 35.9%(プラセボ 0.0% / 0.1%)あるが、費用 2.83bp を越えるものは 1 つも無い**(最大 2.785bp)。 |
...
| [7 銘柄のメイカー検証](mm_all_report.md) | `xyz:MU` の在庫つきメイカー(両側に指値・\|q\|≤1 で FIFO 相殺)を設定ごと 7 銘柄へ当てたもの。1 組あたり損益を**恒等式**で「出したときの半スプレッド / 約定までの mid の動き / 保有中の drift / 手数料」に分解し、入口の門・**無作為の門(帰無対照)**・発注遅延・**執行できる数量**を順に入れる。**出す価値のある銘柄は 1 つも無い**(最良の `xyz:KIOXIA` でも年 \$2.2 万・t=2.36 で非有意、設備費控除前)。 |
| [1 秒足の 6 レジーム判定](regime_report.md) | 1 秒足の mid をボラ 2 × 方向 3 の6 レジームに分け、11 銘柄へ当てたもの。**分布はそのままで時間構造だけ壊したブートストラップ**を帰無対照に、持続性・自己遷移・将来リターン・プラセボで検証する。レジームは状態として実在し、方向つきレジームだけが将来リターンを当てる(Bonferroni 通過 87/198 の全部が R2〜R5)。**気配が疎な銘柄ではボラ判定が壊れ**(`xyz:GOLD` の将来ボラ比 0.73)、**費用を引くと 88 通り全部が負**(最良 −1.35bp/取引、総額 −279 万 USD)。 |
