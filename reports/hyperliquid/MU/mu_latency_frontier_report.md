...
| どこで崩れるか | **1 ブロック(67.3 ms)** で生存率 92.9% → 100 ms で 76.5% | signal decay |
| ブロック周期は | **67.30 ms ± 8.29**(板更新 192 万件の実測。周期格子ではない) | bbo の間隔 |
...
**損益分岐は 50 ms。**そこから 1 ブロック(67.3 ms)ぶん遅れるだけで
t = −2.56 の有意な赤字になる。
...
板更新の間隔を 192 万件で実測すると、**67.30 ms とその整数倍**に山が立つ。
ただし**剛体的な周期格子ではない** — 1 ブロックの間隔は
**中央 67.30 ms・標準偏差 8.29 ms(p10 60.5 / p90 81.2)**と揺らぐ。
整数倍の関係自体は保たれる(実測 ÷ 理論の中央値が 1.001 / 1.978 / 2.990)。

板が動いたブロックどうしの間隔は 1 ブロック 44.4% / 2 ブロック 24.9% /
3 ブロック 12.1% / 4 以上 10.9% で、**半分以上のブロックは板が動かない**。
(これは市場の静けさの統計であって、自分の注文の話ではない。
自分の注文は板を変えるので、載ったブロックは必ず観測できる。)

発注が**次のブロックに入る**なら、実効遅延は

    L_visible ≈ Δ − φ + d      (Δ ≈ 67.3 ms、φ = 直前のブロックからの経過、d = 送信遅延)

φ が一様なら **P(L < 50 ms) ≈ (50 − d) / 67.3**。Δ の揺らぎは
この期待値をほとんど動かさない(1/Δ の期待値で効くだけ)。
...
**2 ブロックかかる場合は L ≥ 60 ms(p10)なので、実質ゼロである。**
...
### 記録する列(生の 24 項目 + 派生 10 項目)

派生量は**その場で確定させて保存する**(後から計算し直すと取り違える)。
...
これで `P(GateSurvived | L)` と `P(GateSurvived | blockPhase)` が直接出る。

**★ blocks_waited の実装を一度書き直した。**最初は「ブロックは周期格子」と仮定して
位相を復元する実装にしたが、実データで検証すると**格子からのずれが中央 15 ms**、
間隔の整数倍と 5 ms 以内で一致するのは 60〜64% しかなく、使いものにならなかった。
**局所中央値で間隔だけ推定する方式**に変えたところ、間隔の復元は 67.30 ms
(実測と一致)、跨いだブロック数の推定は**実測と 100% 一致**した。
`blocks_waited_obs`(板に現れたブロック数・下限)と `block_delta_ms`
(そのとき使った間隔)も並べて残してある。
...
ofi_at_decision, ofi_at_visible, ev_decision, ev_visible,
filled_before_cancel, cancel_send_ns, cancel_effective_ns` + regime, note

(`pred_ev_at_decision` / `pred_ev_at_visible` は `ev_decision` / `ev_visible` に
名前を寄せた。同じ数を 2 列に持たない。)

### ★ EV を実時間で計算できるか — できる(ただし門を作り直す必要がある)

`gate_survived_at_visible = 1[EV_visible > 0]` を記録するには、**probe が実時間で
EV を計算できなければならない**。ところが入口の門の 36 変数のうち

    cancel_10s, add_10s, fragility, resilience, touch_cancel_rate,
    queue_ahead_est_cnt

の 6 個は **L4(注文 1 本ごとの追加・取消)からしか作れず、通常の WS クライアント
(l2Book + trades)では計算できない**。そこでこの 6 個を落とした
**live 版の門**を同じ手順で学習し直した(`build_gate_live.py`)。

| 門 | 通した発注 | 建玉/日 | 1 組あたり往復損益 |
|---|---:|---:|---:|
| 完全版(36 変数) | 238,727 | 62 | −0.049 ± 0.093 bp |
| **live 版(30 変数)** | 260,574 | 55 | **−0.063 ± 0.113 bp** |

**成績はほぼ同じ**(差は誤差の内)。ただし個々の判断の一致は 67.0% で、
完全版が通す 3 件に 1 件は live 版が落とす。**probe には live 版を積めばよい**。
係数は `data/entrygate_live_model_xyz_MU_q1.json` に置いた。
...
3. **φ が一様という仮定**(§3)。ブロックは周期格子ではなく、間隔が
   標準偏差 8.29 ms で揺らぐことを実測した(だから固定格子で位相を復元する
   最初の実装は捨てた)。P(L<50ms) は 1/Δ の期待値でしか効かないので
   表の値は動かないが、φ の分布そのものは probe で確かめる話である。
