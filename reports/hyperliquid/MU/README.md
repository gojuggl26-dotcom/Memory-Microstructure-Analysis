...
この銘柄で作った分析一式を他の所有銘柄(INTC / AMD / KIOXIA / SKHX / SMSN / SNDK)へ
適用した横並びは [7 銘柄の横並び](../cross_coin_report.md) にあります。
...
| [条件つきの出し直しは効かない](mu_requote_report.md) | 気配が動くたびに取り消して並び直す従来の規則に対し、「維持 / 取消して再発注」を条件で選ぶ規則を 13 通り試した。★ **12 通り中 10 通りで従来が Bonferroni 水準で有意に優り**、残り 2 通りも差が検出できない。機構は明快で、**注文を置きっぱなしにすると約定率は 5.10% → 8.01% に上がるのに損益は下がる** — 増える約定は「市場が自分の値段まで掃いてきた」約定、すなわち逆選択である。価値があるのは**最良に居ること**であって**板に居ること**ではない。★ 最初の実装は「約定する注文だけを持ち続ける」= 各時点で判定できない規則になっており +11.2 bp/組(半スプレッドの27 倍)を出した。原因と修正を本文に記録した。 |
| [候補 2 は「約定条件で消える」のではなく「信号が古かった」](mu_cand2_fresh_report.md) | ★ **前報の「候補 2 は見送り」は誤りだった。**板を **100 ms 格子**で作り直すと(発注時点の信号の古さ 2.49 秒 → **0.050 秒**)、段階 1 から勾配が立ち(Q5−Q1 +0.040 → **+0.609**)、往復損益まで残る(+0.212 → **+1.833**)。往復損益との相関は +0.0045 → **+0.1078**。門に足すと日次総額は **+40.63 ± 10.59(t=+3.83)、39 日中 33 日で勝つ**。同じ変数を 2.49 秒古くした版は **−28.37(有意に害)**で、これが最良の帰無対照になっている。先読み検査(1 セル = 100 ms 寝かせ)でも効果の **85%** が残る。ただし五分位は全部負のままで、遅延 65 ms では水準がゼロと区別できない。 |
| [会場間リードラグ — Binance MUUSDT と HL](mu_xv_leadlag_report.md) | 同じ原資産を扱う **2 つの会場**を同じ時計に並べた。★ **Binance が HL を約 200ms 先行する** — 非対称 A(k) はラグ +200ms で **0.0281**、日クラスタ 95% 区間 [+0.0215, +0.0337]、**20 本中 19 本が 0 を跨がない**。「両側がセル内で実更新」に絞っても同一(0.0281)で、LOCF の粘りが作った見かけではない。ピークのまわりが非対称(+0.0185〜+0.0203)なので**一定のクロック差では説明できない**。4 時間帯すべてで同じ形で、**現物が閉まっている夜が最も強い**。★ ただし**乖離の半減期は 179ms** で「古いあいだ待つ」形にはならない。往復損益を目的変数にすると十分位勾配 **+2.98bp**(Δ=130ms)で単調、**1 秒古いと符号が反転**し、**プラセボは 0 を跨ぐ**。★ **それでも 10 段すべて負**(最良 −0.960bp)で、日次総額は最良 −477.7bp/日 — 「何もしない」= 0 に負ける。現物(Nasdaq)を参照先にする案を検討できない理由も記録した。 |
...
| [1 分足の平均回帰(20 分 SMA 基準)](../mrev_report.md)(8 銘柄共通) | $`D_t=\mathrm{mid}_t-F_t`$ の $`F_t`$ を 4 通り(microprice / EWMA / **SMA20** / モデル fair)置き、自己相関 $`\rho(k)`$・AR(1) の半減期・分散比 $`\mathrm{VR}(k)`$ で平均回帰を測る。**同じ欠測・ボラ・ティック幅のランダムウォークを帰無対照に置く**。φ=0.9195 に対しランダムウォークが 0.9232(z=−2.12)。VR も k=60 で 0.915 対 対照 0.989 だが有意ではない。**1 分足ではランダムウォークと区別が付かない**。 |
...
**三候補のまとめ**。解説: [三候補](mu_candidates_report.md)
...
**条件つきの出し直しの検定**。解説: [条件つきの出し直し](mu_requote_report.md)

![xyz:MU 出し直し規則の格子、always との対の差、離れ幅ごとの劣化、約定率と損益が逆を向く 4 枚組](../../../charts/xyz_MU_requote.png)

**候補 2 を新鮮な信号で検定し直す**。解説: [信号が古かった](mu_cand2_fresh_report.md)
...
**米国市場の寄り付きに対して perp は先行するか** — 寄り付き前後 1 秒刻みの perp の動き、寄り値の予測誤差、増分 R²、寄り値との差への追随。解説: [24 時間動く perp は米国市場の寄り付きを先に知っているか](mu_price_discovery_report.md)

![xyz:MU 価格発見の検証。A 寄り付き前後 1 秒刻みの perp の動き、B 寄り値の予測誤差、C 増分 R²、D gap への追随](../../../charts/xyz_MU_pricedisc.png)

**現物が閉まっている時間の動き** — UTC 時刻別の実現ボラティリティ、夜間リターンの一致、差が最大だった日の 1 秒チャート、寄り前 perp の予測力。解説: [24 時間動く perp は米国市場の寄り付きを先に知っているか](mu_price_discovery_report.md)
...
| ファイル | 内容 | 生成スクリプト |
|---|---|---|
| mrev_bars_xyz_MU.parquet | 1 分足(mid / micro / 約定 / スプレッド / OBI)(版管理外) |
...
| [dailypnl_rq_xyz_MU.csv](../../../data/dailypnl_rq_xyz_MU.csv) | 出し直し規則 13 通りの日次総額 | `build_dailypnl.py --set requote` |
| [dailypnl_pairs_rq_xyz_MU.csv](../../../data/dailypnl_pairs_rq_xyz_MU.csv) | always との対の差 | `build_dailypnl.py --set requote` |
| [requote_mech_xyz_MU.csv](../../../data/requote_mech_xyz_MU.csv) | 維持率・発注数・約定率 | 集計 |
| [cand2_stages_xyz_MU_100.csv](../../../data/cand2_stages_xyz_MU_100.csv) | 100 ms 格子での段階分解 | `build_cand2_stages.py --src impact100` |
...
| [cash_MU_1h.csv](../../../data/cash_MU_1h.csv) / [cash_MU_1d.csv](../../../data/cash_MU_1d.csv) | 原資産 MU の時間足(プレ/アフター込み)と日足 | `fetch_cash.py` |
| [cash_NQF_1h.csv](../../../data/cash_NQF_1h.csv) / [cash_SMH_1h.csv](../../../data/cash_SMH_1h.csv) | 統制用の指数先物(NQ=F)と半導体 ETF(SMH)の時間足 | `fetch_cash.py` |
| [hlcandle_xyz_MU_1h.csv](../../../data/hlcandle_xyz_MU_1h.csv) | perp の 1 時間足(公開 API・上場から現在まで 3,203 本) | `fetch_hlcandle.py` |
| [pricedisc_panel_xyz_MU.csv](../../../data/pricedisc_panel_xyz_MU.csv) | 価格発見の日次パネル(90 営業日) | `build_pricedisc.py` |
