...
| [在庫を持つメイカー(maker–maker 相殺・出口方策・往復で学習した入口の門)](mu_inventory_mm_report.md) | 1 約定ごとに手仕舞うのをやめ、**在庫 q を持って反対側のメイカー約定で FIFO 相殺**する事象駆動シミュレータ。**自然相殺はほぼ完全に働く**(60 秒で 98.0%、強制決済 0.012%)が、それでも 1 組 −1.847 bp。**手仕舞いを深く置いても改善しない**(k=1 で数が 3.4 分の 1)。**固定 timeout は短いほど悪い**(強制決済 1 回 1.40〜1.47 bp)。**OBI^inv は将来の値動きではなく「在庫の消しやすさ」を当てる**(五分位で 1 秒相殺率 10.5→19.3%、組損益 −2.87→−0.88)。★**往復損益で入口を学習すると標本外で初めてゼロを跨ぐ**(+0.036 ± 0.084 bp、t=+0.43、130 組/日)。ただし**発注遅延 130 ms を入れると −0.540 ± 0.095 に戻る**。実装の誤り 1 件(待ち行列 0 で値段を無視して約定)と、指標の取り違え 1 件(短い timeout が単価だけ最良に見える)を記録。 |
| [新規発注の実効遅延はどこまで許されるか](mu_latency_frontier_report.md) | 実効遅延そのものは probe を出さないと測れないので、**「いくつなら成立するか」を先に確定**した。**損益分岐は ≈ 50 ms**(遅延ごとに在庫シミュレータを回した標本外 39 日)。**50 ms より内側では板が動かないので条件の 97.3% がそのまま残り**、**1 ブロック(67.3 ms)で 92.9%、100 ms で 76.5% まで崩れる**。壊れる順序は**まず OBI が消え、あとから mid が追いかける**。ブロック周期の実測は 67.30 ms(標準偏差 8.3・周期格子ではない)で、50 ms を切るには**次のブロックに入り、かつ送信遅延 30 ms 未満**が要る(2 ブロックかかると確率 0)。probe の器(`probe_latency.py`・既定で 1 件も送らない)を用意したが、**取引用の鍵が無く実行していない**。 |
...
**1 ティック内側への価格改善**。解説: [価格改善](mu_price_improvement_report.md)
...
| [inv_summary_xyz_MU.csv](../../../data/inv_summary_xyz_MU.csv) | 在庫シミュレータ 16 設定の標本外比較 | `build_inv_summary.py` |
| [unwind_cells_xyz_MU_q1.csv](../../../data/unwind_cells_xyz_MU_q1.csv) | OBI^inv × OFI^inv の 10×10 | `build_unwind.py` |
| [lfrontier_xyz_MU.csv](../../../data/lfrontier_xyz_MU.csv) | 遅延ごとの往復 EV(標本外) | `build_lfrontier.py` |
| [decay_xyz_MU.csv](../../../data/decay_xyz_MU.csv) | 決めてから載るまでのシグナル減衰 | `build_decay.py` |
| [mktpanel.csv](../../../data/mktpanel.csv) | 銘柄×日×時刻の市場状態と EV(3,845 窓) | `build_mktpanel.py` |
| [mktreg.csv](../../../data/mktreg.csv) | 市場状態の回帰(同時 / 1 期先) | `build_mktreg.py` |
...
| inv_days_xyz_MU_*.csv | 設定ごとの日次(版管理は要約のみ) | `build_inventory.py` |
...
uv run python scripts/build_inventory.py --coin xyz:MU --qmax 1 --gate data/entrygate_xyz_MU_q1.parquet
...
uv run python scripts/build_inventory.py --coin xyz:MU --qmax 1 --improve 1 --gate data/entrygate_xyz_MU_q1_imp1.parquet
