...
→ **ではその 8 時間、値段を作っているのはどこなのか。**
続きは [米国が閉まっている夜、値段を動かしているのはどこか](mu_night_discovery_report.md)
(答え: 株価指数先物が 4 分の 3、アジアのメモリ現物が残り。Hyperliquid ではない)。
...
1. **現物の分足が取れない。** この標本期間(2026-05〜08)で無料に取れる原資産の
   最小粒度は **1 時間足**である(2026-09-12 の実測で 1 分足は直近 30 日 =
   2026-08-13 以降、5 分・15 分足は直近 60 日 = 2026-07-15 以降しか返らない)。
   したがって**寄り後 1 時間の中でどちらが先行するかは分解できていない**。
   本レポートが 1 秒粒度で言えるのは perp 側だけである。
   直近 2 か月に限れば 15 分足で組み直せる(詳細は
   [夜の価格発見](mu_night_discovery_report.md) §6)。
