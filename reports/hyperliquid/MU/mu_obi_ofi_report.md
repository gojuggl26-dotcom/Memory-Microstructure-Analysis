...
$`\mathrm{mid} \gt  0`$ なので $`y`$ の符号はリターンの符号と厳密に一致する。
...
Bonferroni の閾値は $`0.05/252 = 1.98\times10^{-4}`$(正規近似で $`|z| \gt  3.86`$)。
