...
> **【訂正 — 2026-09-06】このレポートには誤りが 2 つある。**
> 詳細と検定のやり直しは [`mu_daily_pnl_report.md`](mu_daily_pnl_report.md) にある。
>
> 1. **「単価が非有意だから総額も非有意」は誤り。**日次総額を直接検定すると
>    候補 1 は **+157.7 bp/日、t=+3.62、ブロック bootstrap 95% 区間 [+66.3, +255.9]**
>    で**有意に正**である。単価(日次等重み)と総額(取引重み)は別の量で、
>    片方から他方の有意性は導けない。
> 2. **「候補 2 の信号は約定条件を通ると 20 分の 1」は、比較として厳密でなく、
>    原因の帰属も誤り。**同じ t・同じ側・同じホライズンで揃えると、
>    **段階 1(全候補)の時点で既に勾配が無い**。落ちているのは約定条件ではなく
>    **信号の古さ**である(半減期およそ 250 ms に対し、発注時点での古さは中央 2.49 秒)。
>    したがって**候補 2 は「新鮮な信号」という仮説をまだ検定していない**。
>    **【2026-09-07 追記】検定した。100 ms 格子で作り直すと情報は往復損益まで
>    残り、門に足すと日次総額が +40.63 ± 10.59(t=+3.83)増える。
>    本レポートの「候補 2 = 見送り」は誤りである** →
>    [`mu_cand2_fresh_report.md`](mu_cand2_fresh_report.md)
> 3. §4 の図の注記も、**約定した注文だけの条件付き比較**であって自分の戦略 EV では
>    ないことが読み取りにくかったので直した。
...
| **候補 2**<br>sweep 非対称 + 補充 | 5 秒先リターンとの相関 **−0.070** | ~~相関 +0.0045(20 分の 1)~~ → **100 ms 格子なら相関 +0.1078、門に足して +40.6 bp/日(t=+3.83)** | ~~見送り~~ **→ 採用**(訂正) |
...
| **候補 2** | ~~見送り~~ **→ 採用(2026-09-07 訂正)。**5 秒格子では強度の 3% しか残っておらず、検定になっていなかった。100 ms 格子で作り直すと門に足して **+40.63 ± 10.59 bp/日(t=+3.83)**。詳細は `mu_cand2_fresh_report.md` |
