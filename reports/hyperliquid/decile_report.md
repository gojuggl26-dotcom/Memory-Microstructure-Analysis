...
> **2026-09-13 訂正** — 本節の費用の扱いに 2 点の誤りがあった。
> [7 銘柄 × 84 本の版](decile_all_report.md#5-費用を引くと-1-セルも残らない)で測り直してある。
>
> 1. **費用は銘柄ごとに違う。** 2.83bp は `xyz:MU` の半スプレッドから作った値である。
>    実測すると `xyz:MU` 2.66bp に対し `xyz:INTC` は **3.63bp** で、0.8bp 甘かった。
> 2. **\|D10 − D1\| を往復費用と比べるのは甘い。** D10 に買い・D1 に売りを建てるなら
>    建玉は 2 つなので費用も 2 往復ぶんかかる。正しくは
>    **max(\|D1\|, \|D10\|) > 往復費用** で判定する。
>
> どちらも「費用を越えるセルは無い」という**結論を強める向き**の訂正であり、
> 本節の結論は変わらない。以下の数字はそのまま残す。
