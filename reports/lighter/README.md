...
8. **★maker でも成立しない — しかも理由がテイカーと違う。**指値を BBO へ置いて
   50ms 生イベントから約定を再生すると、純 EV は **−2.743 bp / −$1.45 日**。
   テイカーの敗因は「スプレッドが払えない」だったが、maker の敗因は
   **約定の選別**である。無信号で定期的に quote しても −2.382 bp になり、
   符号を反転すると約定率が 3 倍に跳ね上がる。**指値は価格が自分を突き抜ける
   瞬間にだけ約定する**([maker レポート](lighter_maker_report.md))
...
| レポート | 内容 |
|---|---|
| [★maker 戦略の経済的有意性](lighter_maker_report.md) | 50ms 生イベントから指値注文を再生し、**post-only reject・部分約定・取消遅延中の約定・在庫・rate limit を全部入れた往復純損益**で判定した。結果は **経済的有意性なし** — 純 EV **−2.743 bp**、**−$1.45/日**、日単位 95% 区間 [−2.708, −0.295] で上限も 0 を下回る。**楽観側の待ち行列モデル(Q3)でも −2.209 bp** なので符号は仮定に依存しない。機構は「live 時点の半スプレッド +0.586bp が、発注から約定までの mid の移動 −1.310bp で消える」こと。**約定した買いの 92% で、約定時点の最良買い気配が自分の指値より下**にあった。板の監査では**最良気配の数量減少のうち約定で説明できるのは 0.3〜10% しかない**(行列はほぼ取消で消える)。 |
...
| [会場間リードラグを 100ms へ — MU × Binance](lighter_xv_leadlag_report.md) | 上の 12 ペア測定の続き。**Δ=0.1 秒**に落として「伝播の大半が終わる」250ms の**中**を見た(28 日)。★**100ms の中では Binance が先**(非対称 A(100ms)=**+0.0836** [+0.0702,+0.0971])だが、**200ms 以遠には何も無い**(A(200ms)=−0.0001)。1 秒以上で Lighter 先行という上の結果と**矛盾せず、境目が 100〜200ms にある**ことが判る。時計・絞り・気配/約定値のどれを替えても形は同じ。★**この PC の時計が壊れていることも判った** — 09-09 の再起動で約 480ms 飛び、w32tm(ポーリング 9.1 時間)が直せていない。生の「打刻→受信」は 28 日で **490ms** 振れるが、記録側の clock probe で補正すると **振れは 4.6ms**、片道伝送は Lighter 47.9ms / Binance 48.4ms に収束する。両会場で 0.5ms しか違わないので先行関係の比較には偏りが入らない。★**同じ Binance に対し Hyperliquid `xyz:MU` はラグ 0 の相関 0.0017・ピーク +200ms、Lighter `MU` は 0.6266・ピーク 0** — 2 つの DEX は別物である。Binance 側を約定値に替えた対照で、この差の 9% しか測り方では説明できないことを確認した。 |
