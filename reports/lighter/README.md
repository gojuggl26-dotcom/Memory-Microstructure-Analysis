...
| [会場間リードラグを 100ms へ — MU × Binance](lighter_xv_leadlag_report.md) | 上の 12 ペア測定の続き。**Δ=0.1 秒**に落として「伝播の大半が終わる」250ms の**中**を見た(28 日)。★**100ms の中では Binance が先**(非対称 A(100ms)=**+0.0836** [+0.0702,+0.0971])だが、**200ms 以遠には何も無い**(A(200ms)=−0.0001)。1 秒以上で Lighter 先行という上の結果と**矛盾せず、境目が 100〜200ms にある**ことが判る。時計・絞り・気配/約定値のどれを替えても形は同じ。★**同じ Binance に対し Hyperliquid `xyz:MU` はラグ 0 の相関 0.0017・ピーク +200ms、Lighter `MU` は 0.6266・ピーク 0** — 2 つの DEX は別物である。Binance 側を約定値に替えた対照で、この差の 9% しか測り方では説明できないことを確認した。 |
