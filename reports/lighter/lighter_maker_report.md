...
| 構成 | 純 bp | 判定 |
|---|---:|---|
| **Standard(主)** | **−2.743** | 手数料 0 だが遅い |
| Standard(maker 0ms 仮定) | (§下表) | |
| Premium(ステークなし) | (§下表) | **低遅延では taker 2.80bp を回収できない** |
...
| 主構成 | −2.743 | 1.12% |
| 信号の符号を反転 | (§下表) | |
| 発火時刻を銘柄×日の内側で循環シフト | (§下表) | |
| 定期 quote(無信号・60 秒ごと・向きは乱数) | (§下表) | |

★符号反転は**約定率が跳ね上がる**(MU で 1.42% → 5.83%)。
信号は「不利な側に置かない」方向に効いており、
**方向シグナルとしては本物**である。それでも黒字にならない。
