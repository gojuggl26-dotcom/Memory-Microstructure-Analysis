...
9 件の結果を 4 つのジャンルに分けている。図と詳細は各ファイルにある。

| ジャンル | 何が分かったか | 詳細 |
|---|---|---|
| **板の形状と持続性** | 板の奥行きの「形」(book slope)は将来の金利変化を予測する(174/188 市場)。一方スプレッドの広さそのものや持続時間には予測力が無く、単一市場では標本不足で何も検出できない | [findings_book.md](findings_book.md) |
| **注文流入(Pressure)** | 反対側最良数量で正規化した注文流入は予測どおりに効くが、多くは同一ブロック内で執行できない。最良気配を重くする重み付けで有効標本が 5.1 倍に増える | [findings_flow.md](findings_flow.md) |
| **参加者と報酬設計** | 板は上位 1 口座が発注量の中央 79.7% を握る機械的 MM で、除いても失われる結果は無い。メイカー報酬帯は発注位置を縛っていないのに、帯の内外で符号が逆転する | [findings_participants.md](findings_participants.md) |
| **注文の生存時間** | 94.2% が取消・約定は 5.52% で「約定までの時間の中央値」は存在しない。板レベルの年齢は既存特徴量と独立した予測力を持つ | [findings_lifetime.md](findings_lifetime.md) |
